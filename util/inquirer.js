...
const departmentNames = queries.departmentNames
...
const prompts = {
        ...
        role: [{
                ...
                choices: ["TODO make a tiny func that gives u all the names of depos"]}],
        employee: [{
                type: "input",
                name: "first_name",
...
const subMenus = async (choice) => {
        try {
                ...
                if (choice.includes("view")) {
                        ...
                        const arr = choice.split(" ");
                        const newObject = await inquirer.prompt(prompts[arr[1]]);
                        if (arr[1] === "department") {
                                Add.department(newObject.name);
                                View.departments();
                        } else if (arr[1] === "role") {
                                Add.role(newObject.title, newObject.department_name);
                                View.roles();
                        } else {
                                Add.employee(newObject.first_name, newObject.last_name, newObject.employee_role);
                                View.employees();
                        }
                }
        } 
